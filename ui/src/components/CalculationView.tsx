...
import { Layers, Search, MapPin, Hexagon, Download } from 'lucide-react';
...
export const CalculationView: React.FC<CalculationViewProps> = ({ report, inrRate }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
            <div>
              ...
              <h4 className="text-base font-bold text-slate-900 mt-0.5">Declared Area Basis</h4>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
            <div>
              ...
              <h4 className="text-base font-bold text-slate-900 mt-0.5">Turf Geodesic Area Basis</h4>
            </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
        <div>
          <div className="text-slate-500 font-medium">Unique Features</div>
          <div className="text-xl font-bold text-slate-900 mt-1">{activeCategory.totalUniqueFeatures}</div>
        </div>
        <div>
          ...
          <div className="text-xl font-bold text-blue-600 mt-1">{activeCategory.pointCount}</div>
        </div>
        <div>
          <div className="text-slate-500 font-medium flex items-center gap-1">
          ...
          <div className="text-xl font-bold text-emerald-600 mt-1">{activeCategory.polygonCount}</div>
        </div>
        <div>
          <div className="text-slate-500 font-medium">Polygons &ge; 0.03 ha</div>
          <div className="text-xl font-bold text-slate-900 mt-1">{activeCategory.largePolygonCountCalculated}</div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-bold text-slate-900">Unique Feature Breakdown Table</h3>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                placeholder="Search file, type, properties..."
                className="w-full pl-9 pr-3 py-1.5 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
              />
            </div>

            <button
              onClick={exportReport}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl shadow-xs transition-all shrink-0"
            >
        ...
        <div className="overflow-x-auto max-h-96 overflow-y-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-100 text-slate-600 font-bold border-b border-slate-200 sticky top-0 backdrop-blur-md">
              <tr>
                <th className="py-3 px-4">#</th>
                <th className="py-3 px-4">Source File</th>
                <th className="py-3 px-4">Geometry Type</th>
                <th className="py-3 px-4 text-right">Declared Area</th>
                <th className="py-3 px-4 text-right">Calculated Area</th>
                <th className="py-3 px-4 text-right">Tokens</th>
                <th className="py-3 px-4 text-right">Cost (€ EUR)</th>
            ...
            <tbody className="divide-y divide-slate-100 font-mono">
              {filteredEntries.map((entry, idx) => (
                <tr key={idx} className="hover:bg-slate-50 text-slate-800">
                  ...
                  <td className="py-2.5 px-4 font-semibold text-slate-900 max-w-xs truncate">{entry.firstSeenFile}</td>
                  <td className="py-2.5 px-4">
                    <span className="px-2 py-0.5 rounded text-[11px] bg-slate-100 text-emerald-700 font-semibold border border-slate-200">
                      {entry.featureType}
                    </span>
                  </td>
                  <td className="py-2.5 px-4 text-right text-slate-700">{entry.declaredAreaHa.toFixed(4)} ha</td>
                  <td className="py-2.5 px-4 text-right font-semibold text-emerald-700">{entry.calculatedAreaHa.toFixed(4)} ha</td>
                  <td className="py-2.5 px-4 text-right font-bold text-slate-900">{entry.calculatedTokens}</td>
                  <td className="py-2.5 px-4 text-right text-amber-700 font-semibold">€{entry.calculatedTokens.toFixed(2)}</td>
