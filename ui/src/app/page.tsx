          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Validate GeoJSON compliance, deduplicate geometries, compute tokens & EUR/INR pricing
              </p>
            </div>
          </div>

          {/* Top Right Controls: Currency Bar & Lock Session */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            {/* Simplified Currency Rate Input Box */}
            <CurrencyBar inrRate={inrRate} onRateChange={(rate) => setInrRate(rate)} />

            <button
              onClick={() => setIsAuthenticated(false)}
              className="flex items-center justify-center gap-2 px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 border border-slate-200 rounded-xl text-xs font-semibold transition-all shrink-0"
            >
